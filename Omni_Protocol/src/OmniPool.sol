...
contract OmniPool is IOmniPool, AccessControl, ReentrancyGuardUpgradeable, Pausable {
    ...
    function enterIsolatedMarket(uint96 _subId, address _isolatedMarket) external {
        ...
        require(account.isolatedCollateralMarket == address(0), "OmniPool::enterIsolatedMarket: Already has isolated collateral.");
    ...
    function enterMarkets(uint96 _subId, address[] calldata _markets) external {
        ...
        for (uint256 i = 0; i < _markets.length; ++i) {
            ...
            require(marketConfig.expirationTimestamp > block.timestamp && !marketConfig.isIsolatedCollateral, "OmniPool::enterMarkets: Market invalid.");
    ...
    function enterMode(uint96 _subId, uint8 _modeId) external {
        ...
        require(accountMarkets[accountId].length == 0 && account.isolatedCollateralMarket == address(0), "OmniPool::enterMode: Non-zero market count.");
        ...
        returns (uint256[] memory seizedShares)
    {
        ...
        require(_contains(poolMarkets, _params.liquidateMarket), "OmniPool::liquidate: LiquidateMarket not in pool markets.");
        require(_contains(poolMarkets, _params.collateralMarket) || targetAccount.isolatedCollateralMarket == _params.collateralMarket, "OmniPool::liquidate: CollateralMarket not available to seize.");
        ...
        uint256 amount = IOmniToken(_params.liquidateMarket).repay(_params.targetAccountId, msg.sender, borrowTier, _params.amount);
        (uint256 liquidationBonus, uint256 softThreshold) = getLiquidationBonusAndThreshold(evalBefore.depositAdjValue, evalBefore.borrowAdjValue, _params.collateralMarket);
        { // Avoid stack too deep 
            ...
            seizedShares = IOmniTokenBase(_params.collateralMarket).seize(_params.targetAccountId, _params.liquidatorAccountId, seizeAmount);
        }
    ...
    function getLiquidationBonusAndThreshold(
        ...
        if (_borrowAdjValue > _depositAdjValue) {
            ...
            LiquidationBonusConfiguration memory liquidationBonusConfiguration_ = liquidationBonusConfigurations[_collateralMarket];
            softThreshold = liquidationBonusConfiguration_.softThreshold;
            uint256 pctDiff = Math.ceilDiv(_borrowAdjValue * LIQ_BONUS_PRECISION_SCALE, _depositAdjValue) - LIQ_BONUS_PRECISION_SCALE; // Round up
        ...
        onlyRole(MARKET_CONFIGURATOR_ROLE)
    {
        ...
        if (_marketConfig.expirationTimestamp <= block.timestamp) { revert("OmniPool::setMarketConfiguration: Bad expiration timestamp."); }
        ...
        onlyRole(MARKET_CONFIGURATOR_ROLE)
    {
        ...
        for (uint i = 0; i < _modeConfiguration.markets.length; i++) {
            for (uint j = i + 1; j < _modeConfiguration.markets.length; j++) {
        ...
        onlyRole(SOFT_LIQUIDATION_ROLE)
    {
        if (_softThreshold > MAX_BASE_SOFT_LIQUIDATION || _softThreshold < HEALTH_FACTOR_SCALE) {
            revert("OmniPool::setSoftLiquidation: Soft liquidation health factor threshold cannot be greater than the standard max and must be greater than 1.");
        }
     ...
     */
    function setLiquidationBonusConfiguration(address _market, LiquidationBonusConfiguration memory _config)
        external
        onlyRole(MARKET_CONFIGURATOR_ROLE)
    {
        require(_config.kink <= MAX_LIQ_KINK, "OmniPool::setLiquidationBonusConfiguration: Bad kink for maximum liquidation.");
        require(_config.start <= _config.end && _config.end <= LIQ_BONUS_PRECISION_SCALE, "OmniPool::setLiquidationBonusConfiguration: Bad start and end bonus values.");
