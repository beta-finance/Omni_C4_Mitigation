...
contract TestOmniOracle is Test {
    ...
    function test_SetOracleConfig() public {
        oracle.setOracleConfig(MARKET, IOmniOracle.OracleConfig(MOCK_BAND, IOmniOracle.Provider.Band, 3 hours, 5 hours));
        (address oracleAddress, IOmniOracle.Provider provider, uint32 delay, uint32 delayQuote) = oracle.oracleConfigs(MARKET);
    ...
    function test_RevertZeroSetOracle() public {
        IOmniOracle.OracleConfig memory config = IOmniOracle.OracleConfig(address(0), IOmniOracle.Provider(1), 1 hours, 5 hours);
        vm.expectRevert("OmniOracle::setOracleConfig: Can never use zero address.");
        oracle.setOracleConfig(MARKET, config);

        IOmniOracle.OracleConfig memory config2 = IOmniOracle.OracleConfig(MOCK_BAND, IOmniOracle.Provider(1), 1 hours, 5 hours);
        oracle.setOracleConfig(MARKET, config2);
        vm.expectRevert("OmniOracle::setOracleConfig: Can never use zero address.");
        oracle.setOracleConfig(address(0), config2);

        IOmniOracle.OracleConfig memory config3 = IOmniOracle.OracleConfig(MOCK_BAND, IOmniOracle.Provider(1), 0, 5 hours);
        vm.expectRevert("OmniOracle::setOracleConfig: Invalid delay.");
        oracle.setOracleConfig(MARKET, config3);

        IOmniOracle.OracleConfig memory config4 = IOmniOracle.OracleConfig(MOCK_BAND, IOmniOracle.Provider(1), 5 hours, 0);
        vm.expectRevert("OmniOracle::setOracleConfig: Invalid delay quote.");
        oracle.setOracleConfig(MARKET, config4);
    }

    function testRevertInvalidSetOracle() public {
        IOmniOracle.OracleConfig memory config = IOmniOracle.OracleConfig(MOCK_OTHER, IOmniOracle.Provider(0), 1 hours, 5 hours);
