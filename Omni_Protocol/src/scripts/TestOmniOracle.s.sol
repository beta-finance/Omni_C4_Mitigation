...
contract TestOmniOracle is Script {
    function run() external {
        ...
        oracle.setOracleConfig(
            address(token), IOmniOracle.OracleConfig(SEPOLIA_BAND, IOmniOracle.Provider.Band, 10 hours, 10 hours)
        );
        ...
        oracle.setOracleConfig(
            address(btc), IOmniOracle.OracleConfig(CHAINLINK_BTC_USD, IOmniOracle.Provider.Chainlink, 10 hours, 10 hours)
        );
        ...
        assert(btcPrice == uint256(answer) * (1e36 / (10 ** IChainlinkAggregator(CHAINLINK_BTC_USD).decimals())) / (10 ** IERC20Metadata(address(btc)).decimals()));
